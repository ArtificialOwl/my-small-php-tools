...
trait TNC22Request {
	 ...
	 */
	public function retrieveJson(NC22Request $request): array {
	 ...
	 * @throws RequestNetworkException
	 */
	public function doRequest(NC22Request $request, bool $exceptionOnIssue = true): void {
		$request->setClient(
			$this->clientService()
				 ->newClient()
		);
		...
		foreach ($request->getProtocols() as $protocol) {
			...
			try {
				...
				$request->setResult(new NC22RequestResult($response));
				break;
			} catch (ClientException $e) {
				$request->setResult(new NC22RequestResult(null, $e));
			} catch (Exception $e) {
	 ...
	 * @param NC22Request $request
	 */
	private function generationClientOptions(NC22Request $request) {
		$options = [
	 ...
	 * @throws Exception
	 */
	private function useClient(NC22Request $request): IResponse {
